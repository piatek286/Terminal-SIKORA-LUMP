...
public:
    ...
    void initTable()
    {
        ...
        model = new QStandardItemModel(0,4,this); //2 Rows and 2 Columns
        model->setHorizontalHeaderItem(0, new QStandardItem(QString("Nec/Lump[mm]")));
        model->setHorizontalHeaderItem(1, new QStandardItem(QString("Dł. Nec/Lump[mm]")));
        model->setHorizontalHeaderItem(2, new QStandardItem(QString("Metraż[m]")));
        model->setHorizontalHeaderItem(3, new QStandardItem(QString("Czas")));
        this->setModel(model);
    ...
    void setText(QString *string1, QString *string2, QString *string3)//tableText::setText - visual
    {
        ...
        model->setData(index,*string1);

        index = model->index(this->row-1,1,QModelIndex());
        model->setData(index,*string2);

        index = model->index(this->row-1,2,QModelIndex());
        model->setData(index,*string3);

        index = model->index(this->row-1,3,QModelIndex());
